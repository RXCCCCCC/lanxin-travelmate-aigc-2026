...
- 方案：/api/agent/chat/stream 基于 LangGraph stream(updates) 逐节点推送 SSE stage 事件（中文阶段文案如“查询实时天气与景点”），最后推 final 完整响应；客户端打字指示器实时显示当前阶段，失败自动降级到非流式接口。
...
- 已完成：记忆闭环、个性化规划、主动提醒（3 类触发）、复盘、盲盒、旅拍文案、多人协调后端、真机验证、公网部署、多轮上下文（含追问目的地继承）、打字指示动画、网络降级一键重试、nextActions 动态快捷操作、SSE 流式阶段进度、聊天内嵌行程摘要卡片。
- 进行中：聊天体验优化（本轮）、release 正式签名。
- 已知不足：流式为阶段级而非 token 级；2D 数字人目前为静态立绘+表情状态切换（Live2D 评估后未上，成本收益不匹配）；iOS 未支持（刻意收敛范围）。
