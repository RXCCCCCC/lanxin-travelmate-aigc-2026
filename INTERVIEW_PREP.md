...
- 已完成：记忆闭环、个性化规划、主动提醒（3 类触发）、复盘、盲盒、旅拍文案、多人协调后端、真机验证、公网部署、多轮上下文（含追问目的地继承）、打字指示动画、网络降级一键重试、nextActions 动态快捷操作、SSE 流式阶段进度（聊天页+首页直聊）、聊天内嵌行程摘要卡片、聊天草稿持久化、回到底部按钮、ARCHITECTURE.md 架构文档、OpenAPI 路由摘要与全局异常处理器、仓库瘦身归档。
- 进行中：release 正式签名。
- 已知不足：流式为阶段级而非 token 级（结构化 JSON 需整体校验，阶段级已消除等待焦虑）；2D 数字人目前为静态立绘+表情状态切换（Live2D 评估后未上，成本收益不匹配）；iOS 未支持（刻意收敛范围）。
