...
void main() {
  test('TripReviewService posts context and maps P1 review payload', () async {
    ...
    expect(capturedRequest?.data['profileContext']['travelPace'], 'light');
    expect(capturedRequest?.data['profileContext']['interestTags'], ['夜景']);
    expect(review.route, contains('洪崖洞'));
    expect(review.completedTasks.single['title'], '拍一张夜景');
    expect(
      review.temporaryMemoryPromotions.single['suggestedScope'],
      'longTerm',
    );
  });

  test(
    'TripReviewService returns fallback review when network fails',
    () async {
      final dio = Dio(BaseOptions(baseUrl: 'https://example.test'));
      dio.interceptors.add(
        InterceptorsWrapper(
