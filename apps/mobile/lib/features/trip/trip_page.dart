...
class _AgentTripPlanView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final days = (plan['days'] as List<dynamic>? ?? const [])
        ...
        .toList();
    final risks = (plan['risks'] as List<dynamic>? ?? const []).map((e) => e.toString());
    final matches = (plan['profileMatches'] as List<dynamic>? ?? const []).map((e) => e.toString());
    final alternatives = (plan['alternatives'] as List<dynamic>? ?? const [])
        .whereType<Map<String, dynamic>>()
        .toList();
    final navigationLinks = (plan['navigationLinks'] as List<dynamic>? ?? const [])
        .whereType<Map<String, dynamic>>()
        .toList();
      ...
      children: [
        GlassBox(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Agent 实时规划', style: TextStyle(color: AppTheme.primary, fontSize: 13, fontWeight: FontWeight.w800)),
            const SizedBox(height: AppTheme.spacingSm),
            Text(plan['title']?.toString() ?? '蓝小心规划', style: const TextStyle(color: AppTheme.textPrimary, fontSize: 20, fontWeight: FontWeight.w800)),
            const SizedBox(height: AppTheme.spacingXs),
            Text('${plan['destination'] ?? '目的地'} · ${plan['dateRange'] ?? '行程时间'}', style: const TextStyle(color: AppTheme.textSecondary, fontSize: 13)),
          ]),
        ),
        if (matches.isNotEmpty) ...[
          const _SectionHeader(icon: Icons.psychology_rounded, title: '画像匹配解释'),
          ...matches.map((item) => _SimpleInfoCard(text: item, icon: Icons.auto_awesome_rounded)),
        ],
        ...days.map((day) {
          ...
          return Column(
            ...
            children: [
              Padding(
                padding: const EdgeInsets.fromLTRB(24, 16, 24, 8),
                child: Text(day['dayLabel']?.toString() ?? '行程日', style: const TextStyle(color: AppTheme.textPrimary, fontSize: 17, fontWeight: FontWeight.w800)),
              ),
              ...List.generate(items.length, (index) => _AgentTripItemCard(item: items[index], index: index)),
            ],
          );
        }),
        if (adjustment != null) ...[
          const _SectionHeader(icon: Icons.alt_route_rounded, title: '动态调整'),
          _SimpleInfoCard(text: '${adjustment['trigger']}：${adjustment['suggestion']}', icon: Icons.sync_rounded),
        ],
        if (risks.isNotEmpty) ...[
          const _SectionHeader(icon: Icons.warning_amber_rounded, title: '风险提示'),
          ...risks.map((risk) => _SimpleInfoCard(text: risk, icon: Icons.info_outline_rounded)),
        ],
...
class _AlternativePlanCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GlassBox(
      ...
      padding: const EdgeInsets.symmetric(horizontal: AppTheme.spacingLg, vertical: AppTheme.spacingMd),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(item['title']?.toString() ?? '备选方案', style: const TextStyle(color: AppTheme.textPrimary, fontSize: 15, fontWeight: FontWeight.w800)),
        const SizedBox(height: 6),
        Text(item['summary']?.toString() ?? '', style: const TextStyle(color: AppTheme.textSecondary, fontSize: 13, height: 1.4)),
        if (item['bestFor'] != null) ...[
          const SizedBox(height: 8),
          Text('适合：${item['bestFor']}', style: const TextStyle(color: AppTheme.accent, fontSize: 12, fontWeight: FontWeight.w700)),
        ],
      ]),
    );
  }
  ...
  Widget build(BuildContext context) {
    return GlassBox(
      ...
      child: Row(children: [
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(item['label']?.toString() ?? '打开地图导航', style: const TextStyle(color: AppTheme.textPrimary, fontSize: 15, fontWeight: FontWeight.w800)),
            const SizedBox(height: 4),
            Text(item['url']?.toString() ?? '高德地图跳转链接', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppTheme.textMuted, fontSize: 12)),
          ]),
        ),
      ]),
    );
  ...
  Widget build(BuildContext context) {
    return GlassBox(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(item['time']?.toString() ?? '', style: const TextStyle(color: AppTheme.primary, fontSize: 13, fontWeight: FontWeight.w700)),
            const SizedBox(height: 2),
            Text(item['location']?.toString() ?? '', style: const TextStyle(color: AppTheme.textPrimary, fontSize: 16, fontWeight: FontWeight.w700)),
            const SizedBox(height: AppTheme.spacingXs),
            Text(item['activity']?.toString() ?? '', style: const TextStyle(color: AppTheme.textSecondary, fontSize: 14, height: 1.4)),
            const SizedBox(height: AppTheme.spacingSm),
            Text('因为：${item['reason'] ?? ''}', style: const TextStyle(color: AppTheme.accent, fontSize: 12)),
          ]),
        ),
      ]),
    );
