              ...
              child: agentReview == null
                  ? FutureBuilder<TripReviewPayload>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.hasData) {
                          return _AgentReviewView(review: snapshot.data!.toJson());
                        }
                        if (snapshot.hasError) {
                          return _AgentReviewView(review: TripReviewPayload.fallback().toJson());
                        }
                        return ListView(
...
class _AgentReviewView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final photos = (review['highlightPhotos'] as List<dynamic>? ?? const []).map((e) => e.toString());
    final memories = (review['newMemories'] as List<dynamic>? ?? const []).map((e) => e.toString());
    final tasks = (review['completedTasks'] as List<dynamic>? ?? const [])
        .whereType<Map<String, dynamic>>()
        .toList();
    final states = (review['avatarStatusChanges'] as List<dynamic>? ?? const []).map((e) => e.toString());
    final suggestions = (review['nextTripSuggestions'] as List<dynamic>? ?? const []).map((e) => e.toString());
    final promotions = (review['temporaryMemoryPromotions'] as List<dynamic>? ?? const [])
        .whereType<Map<String, dynamic>>()
        .toList();
      ...
      children: [
        GlassBox(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Agent 旅行复盘', style: TextStyle(color: AppTheme.primary, fontSize: 13, fontWeight: FontWeight.w800)),
            const SizedBox(height: AppTheme.spacingSm),
            Text(review['route']?.toString() ?? '今日路线', style: const TextStyle(color: AppTheme.textPrimary, fontSize: 16, fontWeight: FontWeight.w700, height: 1.4)),
          ]),
        ),
        ...
        if (tasks.isNotEmpty) ...[
          ...
          ...tasks.map((item) => TripReviewCard(item: item['title']?.toString() ?? '已完成旅行任务')),
        ],
        ...
        if (promotions.isNotEmpty) ...[
          ...
          ...promotions.map((item) => TripReviewCard(item: item['title']?.toString() ?? '临时记忆')),
        ],
      ],
    );
  }
