...
- ⬜ 移除主页面对 `apps/mobile/lib/data/mock_data.dart` 的主链路依赖，仅保留测试 fixture/离线降级数据。
- ⬜ 首页、聊天、记忆、规划、提醒、旅拍、复盘继续清理剩余离线/演示状态；画像页已支持真实读取和编辑 `/api/profile/me`，设置页的人格、主动程度、同步策略、通知、语音/文字偏好、自定义 Prompt、隐私摘要、记忆导出、清空记忆、清空当前旅行和撤销云端画像同步已接入真实后端接口。
