...
- 🚧 旅行盲盒任务：后端已支持接受、跳过、完成、状态奖励落库、蓝小心状态事件写入和已完成任务进入复盘；旅拍页已新增端侧真实入口，可对任务执行接受/完成/跳过并同步 `/api/trip/blind-box/tasks/{taskId}/status`，且状态更新必须使用 dashboard 当前真实 `tripId`，无真实旅程时提示先创建或同步旅程。旅拍页已展示完成任务后的好感/默契奖励数值，并为后端 `rewardDeltas` 增加奖励脉冲动画。剩余为复盘页更深入展示。
- 🚧 旅行复盘：后端已持久化生成结果，并可聚合真实路线轨迹、可复盘照片、已完成盲盒任务、提醒历史、蓝小心状态事件、本次旅程记忆沉淀、画像上下文和下次旅行建议；复盘节点已接入真实 Provider `TripReviewOutput` 校验，模型缺失或 schema 无效时保留聚合降级结果；端侧已读取 dashboard/latest review、状态事件和画像上下文。剩余为真实模型效果验收、更多真实状态触发和真机/真实数据验收；无真实上下文时后端复盘只返回空结构，不把固定城市/路线/照片/建议当作产品数据。
