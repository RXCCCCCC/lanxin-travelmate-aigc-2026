...
- 🚧 接入定位、照片选择、相机、通知、麦克风、语音播放、权限拒绝处理；后端 ASR/TTS 任务接口和日志已完成；旅拍页已可通过真实后端登记上传元数据和照片候选，且明确不保存设备本地路径；Android release Manifest 已声明网络、定位、相机、麦克风、通知和图片读取权限，iOS Info.plist 已补相机/相册/麦克风/定位用途说明。Android/vivo 系统相册与相机入口已通过 `MethodChannel('lanxin_travelmate/photo_picker')` 接入，旅拍页会先取得真实 `content://` 选择结果，再仅登记文件名/MIME 和候选元数据，不上传本地 URI；Android/vivo 定位运行时请求已接入并可回填规划当前位置；Android/vivo 系统语音识别与 TTS 播报已通过 `MethodChannel('lanxin_travelmate/voice')` 接入聊天页，麦克风权限会按需请求，识别结果先填入输入框再由用户确认发送，Agent `voiceText`/`replyText` 会尝试调用系统 TTS 播报；Android/vivo 系统通知已通过 `MethodChannel('lanxin_travelmate/notifications')` 接入提醒页，后端提醒返回后可请求 Android 13 通知权限并投递系统通知；语音权限拒绝细化、iOS 照片/定位/通知入口和真机相册/相机/定位/语音/通知行为验收待继续。
- 🚧 补齐加载、空状态、权限拒绝、网络失败、降级说明和重试入口；规划/记忆/提醒/复盘已补真实空状态、加载/错误提示和重试入口，旅拍已有空状态/重试，移动端权限声明静态测试已覆盖 Android/iOS 配置，聊天页已补语音识别失败/TTS 不可用的非阻塞提示，提醒页已补系统通知投递成功/不可用的非阻塞提示，剩余为设备权限拒绝提示细化，以及相机/定位/麦克风/系统语音服务/系统通知真机验收。
