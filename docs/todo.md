...
- 🚧 少量离线降级继续清理；首页改为真实联调入口态，聊天页改为真实 Agent 欢迎态，记忆/规划/提醒/复盘页在无真实数据时展示加载、空状态、错误或重试入口，不再渲染固定杭州/重庆复盘/样例提醒，复盘降级已改为空结构，后端复盘 fallback 在没有真实路线/照片/任务/记忆/建议上下文时不再注入固定重庆样例，且提醒页不再硬编码固定地点；画像页真实读取和编辑 `/api/profile/me`，设置页的人格、主动程度、同步策略、通知、语音/文字偏好、自定义 Prompt、隐私摘要、记忆导出、清空记忆、清空当前旅行和撤销云端画像同步已接入真实后端接口。
...
- 🚧 接入定位、照片选择、相机、通知、麦克风、语音播放、权限拒绝处理；后端 ASR/TTS 任务接口和日志已完成；旅拍页已可通过真实后端登记上传元数据和照片候选，且明确不保存设备本地路径；Android release Manifest 已声明网络、定位、相机、麦克风、通知和图片读取权限，iOS Info.plist 已补相机/相册/麦克风/定位用途说明。端侧系统选择器/相机/通知/定位/语音运行时请求、权限拒绝分支与真实语音能力待继续。
