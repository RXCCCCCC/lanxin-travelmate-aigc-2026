...
- 🚧 旅行盲盒任务：后端已支持接受、跳过、完成、状态奖励落库、蓝小心状态事件写入和已完成任务进入复盘；旅拍页已新增端侧真实入口，可对任务执行接受/完成/跳过并同步 `/api/trip/blind-box/tasks/{taskId}/status`，且状态更新必须使用 dashboard 当前真实 `tripId`，无真实旅程时提示先创建或同步旅程。旅拍页已展示完成任务后的好感/默契奖励数值。剩余为奖励动画和复盘页更深入展示。
