...
- [ ] 确认正式账号体系：手机号/短信/OAuth/队伍演示账号、隐私协议勾选和后端登录接口；当前 App 已提供游客账户与账户设置入口。
...
- [x] 已按用户反馈把首页“聊天历史”改为按行程归类的会话列表，支持新建对话、选择历史会话和进入纯净模式；旧空会话会被过滤，只保留当前空会话或已有消息会话。
- [x] 已把首页“展开”改为可拖拽横线，真机验证可上下调整首页聊天面板占比；纯净模式保留为独立聊天页，使用蓝小心头像，系统返回可回到首页。
- [x] 已把首页“消息”入口改为进入“主动提醒”中心，真机验证入口可响应；提醒页卡片、画像摘要、触发类型和系统通知内容已统一中文展示。
- [x] 已把首页天气卡接入 Android 真机定位和高德天气，真机验证可展示真实区县、天气和温度；后端坐标查询优先使用真实 longitude/latitude。
- [x] 已增强首页直聊状态：Agent 思考中继续输入会加入下一轮思考，停止生成会取消当前请求并显示短暂提示，会话状态由共享控制器维护。
- [x] 已按真机反馈微调首页布局：模式切换气泡放大并补充“切换到”文案，当前旅程、天气和右侧状态气泡上移；纯净模式改为首页内淡入淡出切换，保留顶部四个关键入口，其他状态气泡左右滑出，聊天窗口拉到当前旅程下方并隐藏蓝小心立绘。
- [x] 已修复首页“当前旅程”入口，真机点击可进入行程页；天气卡片改为多行展示真实区县、天气和建议文案，不再省略关键内容。
- [x] 已把首页直聊思考态改为类似 ChatGPT 的持续对话体验：用户消息和头像右对齐，Agent 思考时显示蓝小心头像与“正在思考中......”循环状态，输入框可继续补充信息并保留停止入口。
- [x] 已将底部四个 Tab 做成可收起导航栏，真机验证可在完整 Tab 与紧凑图标栏之间切换，减少首页聊天区域被导航占用。
- [x] 已按用户反馈继续压缩首页气泡占比：删除“等待真实行程”气泡，顶部改为模式切换、消息、聊天历史三入口，天气改为“区县 天气 温度”长条，右侧蓝小心状态改为可收起抽屉，聊天面板初始高度降低，四个快捷入口完整等宽展示。
- [x] 底部导航已改为默认展开、用户点击后完整收起并持久记住选择；全局 Shell 支持左右滑动切换首页/行程/复盘/设置 Tab。真机已验证收起状态与滑动切换，模拟器重启后也可安装最新 APK 验证布局，但当前 AVD 偶发 `Process system isn't responding`，按模拟器环境问题记录。
- [x] 已继续修复首页真机细节：高德天气缓存增加新鲜度限制，天气条恢复 icon 并显示真实温度；首页左上气泡等宽缩小、右侧状态抽屉收窄，聊天面板与底部导航满宽显示，切 Tab 回首页后直聊会话和蓝小心状态保持。
- [x] 设置页已新增“登录账户”入口，支持查看当前游客账户、进入账户设置和显示升级登录提示；正式登录方案仍需人工确认账号体系。
