...
- 🚧 多人出游协调：后端已支持至少 2 名成员真实偏好输入、冲突识别、折中方案、隐私汇总和按 `tripId` 读回；规划页已新增端侧协调入口，可提交成员偏好并展示折中方案/冲突/隐私汇总。剩余为把协调结果深度写入后续规划请求、真实模型增强解释和真实多人数据验收。
