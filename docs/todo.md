...
- 原生平台范围仅保留 Android/vivo：不再维护 iOS、macOS、Windows、Linux、Web 平台壳或相关待办；远端 `dev` 同步后也只应保留 Android 侧相关平台文件，其他端如需恢复必须由负责人重新确认。
...
- 🚧 增加数据库迁移方案，保证 Docker/Postgres 与本地开发库结构一致；已补 Alembic 初始迁移、提醒事件、多人协调、盲盒任务状态、路线轨迹和蓝小心状态事件迁移、Postgres 驱动、Docker 启动前迁移和健康检查；生产迁移回滚策略已补脚本与文档，真实 Postgres 容器启动验收待继续。
- ✅ 替换 `/api/memory`、`/api/profile`、`/api/trip/current` 等 placeholder 路由为真实数据接口。
- 🚧 实现游客用户、设备 ID、会话 ID、最小鉴权和同一用户数据关联；已完成游客、注册、登录、Bearer token、`users/me`、端侧稳定设备 ID、自动游客会话创建和 Dio Bearer 注入；正式 JWT 已完成并保留旧 token 过渡兼容；游客可通过 `/api/auth/upgrade-guest` 原地升级为密码账号并保留同一 `userId` 下的既有数据归属，真实多设备验收待继续。
- 🚧 实现端侧 Drift 与后端云端数据同步：全量同步、选择性同步、冲突检测、合并记录；后端 `push/pull/selected-memory/revoke` 已完成，`push` 支持 `updatedAt` 冲突检测与 `serverWins/clientWins` 策略，`revoke` 支持撤销云端记忆/画像/旅程副本；设置页已接入画像、指定记忆和指定旅程撤销入口，并补齐基础冲突检测与本机覆盖云端入口；端侧已新增 Drift 本地同步队列，记忆新增/编辑/删除会入队，设置页会优先消费 `memory/upsert` 队列和 `memory/delete` 撤销队列，按成功/失败更新状态，并展示最近同步历史；记忆页已支持选择本地记忆后只同步已选条目；App 启动和回到前台时会自动重试 pending 队列。剩余工作集中在正式多设备登录、设备 ID 绑定策略和真实环境同步验收。
- ✅ 增加清空本次旅行数据、清空全部记忆、导出个人数据接口。

人工介入：如果要接入手机号/短信/第三方 OAuth 等正式账号体系，需要人工确认登录方式、隐私策略、旧游客 token 失效策略和多设备合并规则。
...
- 🚧 接入定位、照片选择、相机、通知、麦克风、语音播放、权限拒绝处理；后端 ASR/TTS 任务接口和日志已完成；旅拍页已可通过真实后端登记上传元数据和照片候选，且明确不保存设备本地路径；Android release Manifest 已声明网络、定位、相机、麦克风、通知和图片读取权限；仓库已按 Android/vivo-only 规则移除 iOS/macOS/Windows/Linux/Web 平台壳。Android/vivo 系统相册与相机入口已通过 `MethodChannel('lanxin_travelmate/photo_picker')` 接入，旅拍页会先取得真实 `content://` 选择结果，再仅登记文件名/MIME 和候选元数据，不上传本地 URI；Android/vivo 定位运行时请求已接入并可回填规划当前位置；Android/vivo 系统语音识别与 TTS 播报已通过 `MethodChannel('lanxin_travelmate/voice')` 接入聊天页，麦克风权限会按需请求，识别结果先填入输入框再由用户确认发送，Agent `voiceText`/`replyText` 会尝试调用系统 TTS 播报；Android/vivo 系统通知已通过 `MethodChannel('lanxin_travelmate/notifications')` 接入提醒页，后端提醒返回后可请求 Android 13 通知权限并投递系统通知；定位/麦克风/通知权限拒绝、相册/相机/语音/TTS/通知服务不可用已映射为端侧明确中文提示，Android/vivo 真机相册/相机/定位/语音/通知行为验收待继续。
- 🚧 补齐加载、空状态、权限拒绝、网络失败、降级说明和重试入口；规划/记忆/提醒/复盘已补真实空状态、加载/错误提示和重试入口，旅拍已有空状态/重试，移动端权限声明静态测试已覆盖 Android 配置，聊天页已补语音识别失败/TTS 不可用的非阻塞提示，提醒页已补系统通知投递成功/不可用的非阻塞提示，设备权限拒绝和服务不可用提示已细化到定位、麦克风、通知、相册、相机、语音识别和 TTS；剩余为相机/定位/麦克风/系统语音服务/系统通知真机验收。

人工介入：Android/vivo 真机或稳定模拟器权限测试；提供真实照片素材；确认通知、定位、相册、相机、麦克风权限文案；验收系统通知权限弹窗、通知展示效果、系统语音识别服务和 TTS 中文播报效果。
...
- 🚧 个性化规划闭环：创建旅行、生成方案、解释画像匹配、用户修改后重新规划、保存旅程；后端已支持同一 `tripId` 重规划、保存日期/预算/同行人/风格并把预算/交通方式/偏好/重规划原因写入 `profileMatches/risks`，端侧修改入口已完成，可从已生成方案回填表单并重新规划；真实模型增强待继续。
...
- ✅ 旅行盲盒任务：后端已支持接受、跳过、完成、状态奖励落库、蓝小心状态事件写入和已完成任务进入复盘；旅拍页已新增端侧真实入口，可对任务执行接受/完成/跳过并同步 `/api/trip/blind-box/tasks/{taskId}/status`，且状态更新必须使用 dashboard 当前真实 `tripId`，无真实旅程时提示先创建或同步旅程。旅拍页已展示完成任务后的好感/默契奖励数值并增加奖励脉冲动画，复盘页已展示任务状态、奖励、影响、备注和完成时间。
- 🚧 旅行复盘：后端已持久化生成结果，并可聚合真实路线轨迹、可复盘照片、已完成盲盒任务、提醒历史、蓝小心状态事件、本次旅程记忆沉淀、画像上下文和下次旅行建议；复盘节点已接入真实 Provider `TripReviewOutput` 校验，模型缺失或 schema 无效时保留聚合降级结果；端侧已读取 dashboard/latest review、状态事件和画像上下文，并以详情卡展示已完成任务、提醒回看和临时记忆沉淀建议。剩余为真实模型效果验收、更多真实状态触发和真机/真实数据验收；无真实上下文时后端复盘只返回空结构，不把固定城市/路线/照片/建议当作产品数据。
- 🚧 多人出游协调：后端已支持至少 2 名成员真实偏好输入、冲突识别、折中方案、隐私汇总和按 `tripId` 读回；规划页已新增端侧协调入口，可提交成员偏好并展示折中方案/冲突/隐私汇总，生成规划时会把 `groupCoordination` 写入 `/api/trip/plan` 并进入 `planningInputs/profileMatches`；端侧结果卡已消费 `privacySummary.sensitiveMemberDetailsHidden/sensitiveMemberCount/publicRule`，只展示汇总依据和敏感详情已隐藏提示。剩余为真实模型增强解释和真实多人数据验收。
- 🚧 蓝小心表达：已有多状态 PNG 素材并接入 Flutter assets；`AvatarState` 已支持后端状态别名、状态事件到视觉状态映射，`AvatarDisplay` 已按状态应用不同浮动/缩放动作参数。剩余为人工确认最终动态素材，并在拿到 GIF/WebP/Lottie 后替换对应资产。
...
- 🚧 敏感记忆识别策略：身体状态、位置、同行人信息已标记 `sensitive`，饮食忌口已标记 `personal`，均要求显式确认；端侧聊天记忆候选已保留 `sensitivity/requiresExplicitConsent` 并展示敏感/个人候选确认提示，最终公开隐私文案待人工确认。
- 🚧 云端请求参数最小化和脱敏；后端模型调用日志已脱敏密钥、Token、密码、用户原文和超长文本，照片候选/上传元数据已避免保存和返回设备本地 URI/路径；端侧旅拍上传元数据已不再发送设备本地 `localPath`，并新增静态测试禁止运行时代码输出 `print/debugPrint/developer.log`；真实照片内容和真机音频权限链路的最终审计待人工验收；后端音频工具调用已按 Bearer 用户写入审计日志。
- 🚧 多人模式默认隐藏个人敏感偏好，仅展示汇总后的协调依据；后端多人协调接口已隐藏敏感偏好原文，端侧多人协调结果卡已展示敏感详情隐藏状态和汇总展示规则，公开隐私文案待人工确认。
...
- 🚧 Docker Compose 已配置 API 指向 Postgres、Postgres healthcheck、API 启动前 `alembic upgrade head`；已新增 `scripts/docker_compose_preflight.py` 静态检查 compose、Postgres healthcheck、API 迁移启动命令，并可在本机有 Docker 时调用 `docker compose config`。剩余为人工/CI 环境执行 `docker compose up --build` 验证真实容器初始化。
...
- 🚧 完成 PPT/答辩材料，突出创新性、应用价值、完成度和大模型应用说明；已新增 `docs/handoff/presentation-outline.md` 作为 12 页答辩大纲，剩余为人工加入队伍信息、真实截图、视频链接、最终设计排版和审稿。
- 🚧 生成 App 截图、接口截图、架构图、toolTrace 示例和风险说明页；已新增 `docs/handoff/demo-evidence-pack.md`，包含系统架构图、Agent 主链路图、toolTrace 示例、风险说明页和必截画面清单。剩余为人工在真机/Swagger/接口工具中截取真实画面。
- 🚧 检查提交包：作品名称、团队介绍、代码仓库、PPT、视频、文档一致；已新增 `docs/handoff/submission-checklist.md`，剩余为人工填写团队/平台信息、上传材料并最终确认。
...
- 2026-06-21 auth-scope follow-up: `/api/profile/me`, `/api/memory/capsules`, `/api/memory/export`, memory clear, and `/api/agent/chat` now resolve the Bearer-token user when `userId` is omitted or still equals the compatibility default `guest`; explicit non-guest `userId` remains supported for legacy scripts/tests. Remaining work: validate real multi-device login and merge behavior.

- 2026-06-21 data-route auth-scope follow-up: photo candidate/upload/copywriting routes, sync push/pull/selected/revoke routes, and tool-call userId echoes now use the same Bearer-token-first ownership rule. Remaining work in this ownership track: validate real multi-device login and merge behavior.

- 2026-06-21 trip auth-scope follow-up: trip plan/current/dashboard/review/reminder/history/group/route/avatar/blind-box endpoints now use Bearer-token-first ownership for omitted/default `guest` userId while preserving explicit non-guest compatibility. Remaining ownership item: validate real multi-device login and merge behavior.
