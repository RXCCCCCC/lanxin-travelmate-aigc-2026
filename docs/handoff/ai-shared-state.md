...
- 暂停点：用户反馈”相册功能正常，但首页语音输入没有任何反馈和用处”。已按系统调试流程完成初步定位，根因高度明确：独立聊天页 `apps/mobile/lib/features/chat/chat_page.dart` 已通过 `VoiceInteractionService.listenOnce()` 绑定 `lanxin_travelmate/voice` 原生通道，并有”已填入语音识别文本/未识别到语音内容”等反馈；Android `MainActivity.kt` 也已实现 `startVoiceInput`、麦克风权限申请和 `RecognizerIntent` 回填。但首页 `apps/mobile/lib/features/home/home_page.dart` 的 `_ChatGlassPanel` 输入栏左侧只是静态 `Icons.mic_rounded`，外层点击只会 `onFocusInput` 聚焦输入框，没有 `IconButton`、没有调用 `VoiceInteractionService`、没有提示状态。因此下一步应先为 `HomePage` 写失败 widget test：点击首页麦克风后应调用语音服务、把识别文本填入首页输入框并展示反馈；随后把首页接入 `VoiceInteractionService`，复用聊天页失败文案和禁用态，最后跑 `flutter test test/home_dashboard_test.dart test/chat_page_integration_test.dart` 及 `flutter analyze`。
- 首页语音输入已修复（分支 `feature/home-voice-input`）：`HomePage` 新增可选 `VoiceInteractionService` 参数，`_ChatGlassPanel` 的静态麦克风图标已替换为 `Tooltip('语音输入') + IconButton`，对接 `_listenAndFillHomeInput()` 方法；监听中显示”正在听你说话...”、成功填入显示”已填入语音识别文本，可编辑后发送”、失败回退到 `lastFailureMessage` 或”未识别到语音内容，请确认麦克风权限或使用文字输入”。验证通过：`flutter test test/home_dashboard_test.dart test/chat_page_integration_test.dart` 结果 12 passed（含新增 `HomePage microphone fills recognized speech into input`），`flutter build apk --debug` 成功并已安装到真机 `NBGYVG8T6TCAJNGY`。人工验收仍需在真机点击首页麦克风确认系统语音识别弹窗、识别回填和失败提示。
- vivo ASR 后端代理已实现（分支 `feature/vivo-asr-voice-input`）：OPPO 手机无系统 `RecognizerIntent` 支持，改为 PCM 录音 + 后端代理 vivo ASR WebSocket。新增 `app/services/asr_client.py`（vivo ASR WebSocket 客户端），`POST /api/audio/asr/recognize` 接受 JSON `{“audio_base64”: “...”, “userId”: “guest”}` → 后端解码 base64 → WebSocket 流式发送 PCM → 返回识别文本。Android `MainActivity.kt` 新增 `startPcmRecording`/`stopPcmRecording` MethodChannel（AudioRecord 16k/16b mono PCM，录音线程自动停止，返回 base64）。Dart `VoiceInteractionService.listenOnce()` 改为 8 秒自动录音 + 后端 ASR 代理。Flutter 测试 12 passed，后端非 provider 测试 13 passed。APK 已构建安装到真机。接入点相同：用户点击首页麦克风 → “正在听你说话...” → 自动录音 → vivo ASR 识别 → 文本填入输入框。
