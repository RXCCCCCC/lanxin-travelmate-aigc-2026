...
- Android release Manifest 已声明网络、定位、相机、麦克风、通知、图片读取和 Android 12 及以下相册读取兼容权限。
- 当前仓库只维护 Android/vivo 原生平台壳；非 Android 平台权限说明不再作为本仓库交付范围。
- 这些声明只表示安装包具备申请权限的基础配置；运行时请求、拒绝后的降级交互和真机验收仍需继续。
