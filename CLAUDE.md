...
- 2026-06-20 blind-box follow-up: `PhotoExperienceService.updateBlindBoxTaskStatus()` posts to `/api/trip/blind-box/tasks/{taskId}/status`; `PhotoPage` now renders accept/complete/skip controls for blind-box tasks and updates local task state from the backend response. Validation passed: `flutter analyze`; backend blind-box/avatar event tests passed with `uv run pytest tests/test_blind_box_task_state.py tests/test_avatar_state_events.py -q`. Flutter widget tests were not rerun because the local Flutter test runner timed out repeatedly in this session.
