...
- 2026-06-20 group coordination follow-up: `TripGroupService` calls `/api/trip/group/coordinate`; `TripPage` now exposes a two-member group preference coordination entry before planning, displays compromise pace/budget, conflicts, and privacy summary, and keeps sensitive raw preference handling delegated to the backend response contract. Validation passed: `flutter analyze`; `$env:NO_PROXY='localhost,127.0.0.1,::1'; flutter test test\trip_group_service_test.dart test\trip_page_integration_test.dart`.
<!-- gitnexus:start -->
