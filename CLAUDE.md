...
- `services/api/`：FastAPI + LangGraph 后端，使用 `uv` 管理依赖；未配置真实模型时默认 Mock Provider，Docker 本地演示会读取 `services/api/.env` 的真实模型/高德配置；天气/POI/步行/驾车/公交/混合路线已接高德 Provider，无 Key 时显式 `provider=unconfigured` 降级。
- `docs/`：统一文档目录，按 `product/`、`engineering/`、`handoff/` 分层维护。
- infra/docker-compose.yml：api + postgres 本地编排，nginx 为占位服务。
- 后端已新增 Alembic 初始迁移：services/api/alembic.ini、services/api/migrations/；API 容器通过 LANXIN_DATABASE_URL=postgresql+psycopg://... 连接 Postgres，并在启动前执行 uv run alembic upgrade head。Alembic revision id 必须保持不超过 32 字符，以兼容默认 Postgres `alembic_version.version_num`。
...
- 2026-07-03 real-provider schema follow-up: `TripPlanningOutput.alternatives` now normalizes string alternatives into dictionaries, memory candidate `recommendedScope` accepts real-model aliases such as `shortTerm`, and final companion chat responses merge model chat candidates with extracted/rule memory candidates instead of overwriting them. `infra/docker-compose.yml` now loads `services/api/.env` for real model/Amap settings while overriding only the Postgres database URL. Validation passed: `uv run pytest tests/test_model_outputs.py tests/test_model_providers.py tests/test_agent_api.py -q`, `uv run python -m py_compile app\agents\travelmate\schemas\model_outputs.py app\agents\travelmate\nodes\real_nodes.py`, `docker compose -f infra/docker-compose.yml up --build -d`, and Docker API smoke on `127.0.0.1:8000` returned memory extraction, trip planning, trip review, and companion chat with `fallback=false`.
- 2026-06-29 real-provider acceptance follow-up: `uv run python scripts\real_provider_smoke.py` passes with Amap weather/walking route `provider=amap fallback=false` and OpenAI-compatible model connectivity `ok=True`; additional direct Amap POI/driving/transit/mixed route checks also return `provider=amap fallback=false`. `OpenAICompatibleProvider` and `LanxinModelProvider` now inject scenario-specific JSON contracts into the system prompt, and `/api/photo/copywriting` normalizes common real-model aliases (`wechat/xhs/travelDiary/vlog`) into the public schema. Real photo copywriting smoke returns `provider=openai_compatible fallback=false`. Agent, trip plan/review, and photo copywriting model calls now persist into `/api/audit/model-calls` with redacted request summaries. Remaining risk is external provider outage or timeout during final demo, so final acceptance should still verify five real scenarios with `fallback=false`.
...
- 2026-07-03 Docker compose startup follow-up: shortened long Alembic revision ids so fresh Postgres migrations fit default `alembic_version.version_num varchar(32)`, added a regression test for that limit, and validated `docker compose -f infra/docker-compose.yml up --build -d`: Postgres became healthy, API started after `alembic upgrade head`, `uv run alembic current` reported `0007_tool_call_user_id (head)`, and `/api/health` returned ok.
